...
-- Session-wide log of every write diff opened; entries are never removed so
...
local function setup_diff_highlights()
    ...
    if defined and defined.bg then return end  -- already set up this session
...
-- send_fn(action, edited_args_json?) is called when the user makes a decision.
function M.open_write_diff_editor(channel_id, payload, send_fn, opts)
    opts = opts or {}
    ...
    -- Record a new history entry unless this call is a re-open from the picker.
    ...
    if not opts.skip_history then
        ...
        table.insert(diff_history, history_entry)
    else
        -- Re-opening a historical diff: find the original entry so approve/deny
        -- still update the right status field.
        history_entry = opts.history_entry
    end
    ...
    vim.wo[mid_win].winbar   = preview.note
        and string.format(" RESULT \226\156\143 (edit here)  \226\148\130  %s  %s  \226\154\160 %s", display, diff_stats, preview.note)
    ...
    local function map_diff_keys(buf)
        ...
        vim.keymap.set("n", "<leader>h", function() open_hunk_picker(current_lines, proposed_lines, mid_win) end, vim.tbl_extend("force", opts, { desc = "Browse hunks in Telescope" }))
        vim.keymap.set("n", "zm",        "zM",     vim.tbl_extend("force", opts, { desc = "Refold all unchanged sections" }))
        vim.keymap.set("n", "<leader>s", function() M.open_diff_history() end, vim.tbl_extend("force", opts, { desc = "Browse session diff history" }))
    end
...
local function open_revert_diff_editor(path, original_lines)
    ...
    vim.wo[right_win].winbar =
        string.format(" \226\143\135 ORIGINAL  %s  (pre-session) ", path)
...
local function open_history_diff_view(entry)
    ...
    vim.cmd("vsplit")
    local right_buf = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_buf_set_lines(right_buf, 0, -1, false, entry.proposed_lines or {})
    ...
    vim.wo[right_win].winbar = string.format(
        ...
        entry.path or "", entry.seq or 0, entry.added or 0, entry.removed or 0, entry.status or "?"
    )
    vim.wo[right_win].winhighlight =
        "DiffAdd:KraDiffAdd,DiffDelete:KraDiffDelete,DiffChange:KraDiffChange,DiffText:KraDiffText"
    ...
    local function close_view()
        ...
        if vim.api.nvim_tabpage_is_valid(diff_tab) then vim.cmd("tabclose") end
        if vim.api.nvim_tabpage_is_valid(original_tab) then
            vim.api.nvim_set_current_tabpage(original_tab)
        end
    end
...
function M.open_diff_history()
    local ok, pickers = pcall(require, "telescope.pickers")
    if not ok then
        vim.notify("Telescope is required for the diff history picker.", vim.log.levels.WARN, { title = "Diff History" })
        return
    end
    ...
    pickers.new({}, {
        ...
        finder = finders.new_table({
            ...
            entry_maker = function(item)
                ...
                if item.kind == "original" then
                    ...
                    label = string.format("#%-3d %s  %-40s  +%d -%d  (%s)  [%s]",
                        e.seq, icon, e.path, e.added, e.removed, e.timestamp, e.status)
                end
        ...
        previewer = previewers.new_buffer_previewer({
            ...
            define_preview = function(self, sel_entry)
                ...
                local h = item.entry
                local a_lines, b_lines = h.current_lines, h.proposed_lines
                local hunks = vim.diff(
                    table.concat(a_lines, "\n") .. "\n",
                    table.concat(b_lines, "\n") .. "\n",
                    { result_type = "indices" }
                ...
                for _, hunk in ipairs(hunks) do
                    ...
                    table.insert(lines, {
                        string.format("@@ -%d,%d +%d,%d @@", a_start, a_count, b_start, b_count),
                        "Comment"
                    })
                    for j = math.max(1, a_start - ctx), a_start - 1 do
                        table.insert(lines, { "  " .. (a_lines[j] or ""), "Normal" })
                    end
                    for j = a_start, a_start + a_count - 1 do
                        table.insert(lines, { "- " .. (a_lines[j] or ""), "DiffDelete" })
                    end
                    for j = b_start, b_start + b_count - 1 do
                        table.insert(lines, { "+ " .. (b_lines[j] or ""), "DiffAdd" })
                    end
                    for j = b_start + b_count, math.min(#b_lines, b_start + b_count + ctx - 1) do
                        table.insert(lines, { "  " .. (b_lines[j] or ""), "Normal" })
                    end
                    table.insert(lines, { "", "Normal" })
                end
                if #lines == 0 then
                    lines = {{ "(no differences)", "Comment" }}
                end
                local text = vim.tbl_map(function(l) return l[1] end, lines)
                vim.api.nvim_buf_set_lines(self.state.bufnr, 0, -1, false, text)
                for idx, l in ipairs(lines) do
            ...
            actions.select_default:replace(function()
                ...
                if sel then
                    ...
                    if item.kind == "original" then
                        ...
                        if orig then open_revert_diff_editor(item.path, orig) end
                    else
