...
const INVESTIGATE_PARAMETERS: Record<string, unknown> = {
    ...
    properties: {
        ...
        hint: {
            type: 'string',
            description: 'Optional steering: known paths, symbol names, prior findings, user context. The investigator only sees `query` + `hint` — be generous.',
        },
        scope: {
            type: 'string',
...
export function createInvestigateTool(opts: CreateInvestigateToolOptions): LocalTool {
    ...
    return {
        ...
        description: [
            'Research tool for LOCATION/FLOW questions: "where is X handled?", "what does Y do?",',
            '"how does Z work?", "which files implement W?". Returns a curated, evidence-backed',
            'synthesis (verified excerpts + short summary) from a cheaper sub-agent model instead',
            'of dumping raw files into your context.',
            '',
            'NOT for diagnosis, root-cause, bug-hunting, or design judgement — those are YOUR job.',
            'Use it to gather evidence (locations, call sites, control flow); reason about it yourself.',
            '',
            'Call BEFORE doing your own broad searches/reads. Pass any known context via `hint`',
            '(paths, symbols, prior findings) — it only sees `query` + `hint`. May be called at any',
            'point in the turn. Skip only for trivial lookups where you know the exact file+range.',
            '',
            'A `query` may bundle tightly-related sub-questions sharing one scope; do NOT mix unrelated',
            'topics. Only ONE investigation at a time — wait for in-flight calls to finish.',
        ].join(' '),
...
function buildInvestigatorSystemPrompt(settings: InvestigatorRuntime['settings']): string {
    return [
        'You are an investigator sub-agent. Answer ONE research question with concrete,',
        'evidence-backed findings. NOT for code changes. NOT for diagnosis, root-cause,',
        'bug-hunting, or design judgement — your scope is LOCATION and FLOW (where code lives,',
        'what it does, how it connects). If asked to "find the bug" or "figure out why X is',
        'broken", DO NOT diagnose — locate the relevant code paths, return them as evidence,',
        'and state in `summary` that the orchestrator should take the diagnosis step.',
        '',
        'You are a small fast model. Bias toward the obvious answer; stop as soon as you can',
        'answer confidently. Do not exhaustively map the codebase.',
        '',
        'Workflow:',
        '  1. Search for the most relevant code/docs. If caller passed `scope`, treat it as a',
        '     HARD restriction — do not search outside it.',
        '  2. Capture short verbatim excerpts at exact line ranges.',
        `  3. Cap each excerpt at ${settings.maxExcerptLines} lines; cap total evidence at ${settings.maxEvidenceItems} items. Prefer fewer high-signal excerpts.`,
        '  4. Call `submit_result` once (summary + evidence + confidence). Then briefly acknowledge',
        '     and stop. Do NOT call any tool after `submit_result`, never call orchestrator-only',
        '     end-of-turn tools.',
        '',
        'Quality: every excerpt MUST match file content at stated lines. `confidence` ∈ {high,',
        'medium, low} — honest. If you could not find what was asked, return `low` and say so.',
