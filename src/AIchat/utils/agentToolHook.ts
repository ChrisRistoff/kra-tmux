...
export async function buildWritePreview(toolArgs: unknown, workspacePath: string): Promise<ToolWritePreview | undefined> {
    ...
    if (editLinesRequest) {
        ...
        if (editLinesRequest.startLines && editLinesRequest.endLines && editLinesRequest.newContents) {
            // Multi-edit (array) form — apply edits bottom-to-top, same as the MCP server.
            const { startLines, endLines, newContents } = editLinesRequest;
            ...
            note = editLinesRequest.newContent! === ''
...
export async function handlePreToolUse(
    ...
    if (input.toolName.includes('edit_lines') && preview?.applyStrategy === 'edit-tool') {
        if (userNewStr !== undefined) {
            ...
            return {
                permissionDecision: 'allow',
                modifiedArgs: { ...baseArgs, start_line: 1, end_line: 999999, new_content: userNewStr },
            };
        }
        return { permissionDecision: 'allow' };
    }
