...
_kra_completions() {
    ...
    if [[ "$prev" == "git" ]]; then
        COMPREPLY=( $(compgen -W "create-branch open-pr view-changed restore cache-untracked retrieve-untracked hard-reset log stash stash-drop-multiple conflict-handle" -- "$cur") )
    elif [[ "$prev" == "tmux" ]]; then
